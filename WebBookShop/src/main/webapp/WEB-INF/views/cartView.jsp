						...
						<div class="row">
							<c:choose>
								<c:when test="${empty sessionScope.currentUser}">
									<p>
										Vui lòng <a href="${pageContext.request.contextPath}/signin">đăng nhập</a> để sử dụng giỏ hàng.
									</p>
								</c:when>
								...
								<c:otherwise>
									<c:if test="${empty cartItems}">
										<p>
											Giỏ hàng của bạn đang trống. <a href="${pageContext.request.contextPath}/">Mua sắm ngay</a>
										</p>
									</c:if>
																...
																<td>
																	<div class="d-flex align-items-center">
																		<c:choose>
																			...
																			<c:otherwise>
																				<img src="${pageContext.request.contextPath}/image/${item.product.imageName}"
																					width="60" height="60" class="me-2" />
																			</c:otherwise>
																		</c:choose>
																		<a
																...
																<td>
																	<form action="${pageContext.request.contextPath}/cartItem" method="post"
																		...
																		<input type="hidden" name="action" value="update" /> <input type="hidden"
																			name="cartItemId" value="${item.id}" /> <input type="number"
																			name="quantity" value="${item.quantity}" min="1" style="width: 70px" />
																		<button type="submit" class="btn btn-sm btn-primary">Cập nhật</button>
																	</form>
																</td>

																<td>
																	<fmt:formatNumber value="${item.product.price * item.quantity}" pattern="#,##0" />
																	₫
																	<c:set var="tempTotal"
												...
												<div class="card-body border-top">
													<form action="${pageContext.request.contextPath}/cart" method="post">
														...
														<div class="form-check">
															<input class="form-check-input" type="radio" name="deliveryMethod" value="1" checked />
															<label class="form-check-label">Giao tiêu chuẩn</label>
														</div>
														<div class="form-check mb-2">
															<input class="form-check-input" type="radio" name="deliveryMethod" value="2" /> <label
											...
											<div class="card mb-3">
												<div class="card-body">
													...
													<dl class="row mb-0">
														...
														<dd class="col-6 text-end">
															<strong>
																<fmt:formatNumber value="${tempTotal + 30000}" pattern="#,##0" />₫
															</strong>
														</dd>
													</dl>
												</div>

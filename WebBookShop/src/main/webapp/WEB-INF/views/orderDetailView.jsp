                  ...
                  <main class="col-md-9">
                     <article class="card mb-4">
                        ...
                        <header class="card-header">
                           <strong class="d-inline-block me-4">Mã đơn hàng: ${requestScope.order.id}</strong> <span>Ngày mua:
                        ...
                        <div class="card-body pb-0">
                           <div class="row">
                              <div class="col-lg-8">
                                 ...
                                 <p class="lh-lg">
                                    ${sessionScope.currentUser.fullname} <br> Số điện thoại: ${sessionScope.currentUser.phoneNumber} <br>
                                    Địa chỉ: ${sessionScope.currentUser.address}
                                 </p>
                              </div>
                              <div class="col-lg-4">
                                 <h6 class="text-muted">Hình thức thanh toán</h6>
                                 <span class="text-success"> <i class="fab fa-lg fa-cc-visa"></i> ${requestScope.order.deliveryMethod == 1 ? "Giao tiêu chuẩn" : "Giao nhanh"}
                                 </span>
                                 <p class="lh-lg">
                                    Tạm tính:
                                    <fmt:formatNumber pattern="#,##0" value="${requestScope.tempPrice}" />
                                    ₫ <br> Phí vận chuyển:
                                    <fmt:formatNumber pattern="#,##0" value="${requestScope.order.deliveryPrice}" />
                                    ₫ <br> <strong> Tổng cộng: <fmt:formatNumber pattern="#,##0"
                                       ...
                                       <td>
                                          <figure class="itemside">
                                             <div class="float-start me-3">
                                                <c:choose>
                                                   <c:when test="${empty orderItem.product.imageName}">
                                                      <img width="80" height="80" src="${pageContext.request.contextPath}/img/280px.png"
                                                         alt="280px.png">
                                                   </c:when>
                                                   <c:otherwise>
                                             ...
                                             <figcaption class="info">
                                                <a href="${pageContext.request.contextPath}/product?id=${orderItem.product.id}" target="_blank">
                                          ...
                                          <div class="price-wrap">
                                             <c:choose>
                                                <c:when test="${orderItem.discount == 0}">
                                                   <span class="price"> <fmt:formatNumber pattern="#,##0" value="${orderItem.price}" />₫
                                                   </span>
                                                </c:when>
                                                <c:otherwise>
                                                   <div>
                                                      <span class="price"> <fmt:formatNumber pattern="#,##0"
                                                            value="${orderItem.price * (100 - orderItem.discount) / 100}" /> ₫
                                                      </span> <span class="ms-2 badge bg-info"> - <fmt:formatNumber pattern="#,##0"
                                                            value="${orderItem.discount}" />%
                                                      </span>
                                                   </div>
                                                   <small class="text-muted text-decoration-line-through"> <fmt:formatNumber pattern="#,##0"
                           ...
                           <c:choose>
                                 ...
                                 <form action="${pageContext.request.contextPath}/orderDetail" method="post" class="d-inline">
                                    ...
                                    <c:choose>
                                       <c:when test="${empty requestScope.confirmCancel}">
                                          <button type="submit" name="action" value="requestCancel" class="btn btn-danger me-2">Hủy đơn hàng
                                          </button>
                                       </c:when>

                                       <c:otherwise>
                                          <span>Bạn có chắc chắn muốn hủy đơn?</span>
                                          <button type="submit" name="action" value="confirmCancel" class="btn btn-danger me-2">Xác nhận hủy
                                          </button>
                                          <button type="submit" name="action" value="cancelCancel" class="btn btn-secondary">Hủy thao tác</button>
                                       </c:otherwise>
                                    </c:choose>
                                 </form>
                              </c:when>
                              ...
                              <c:otherwise>
                                 <form action="${pageContext.request.contextPath}/cartItem" method="post" class="d-inline">
                                    <input type="hidden" name="action" value="add"> <input type="hidden" name="userId"
                                       value="${sessionScope.currentUser.id}"> <input type="hidden" name="productId"
                                       value="${requestScope.orderItems[0].product.id}"> <input type="number" name="quantity"
                                       class="form-control w-auto d-inline" value="1" min="1" max="${requestScope.orderItems[0].product.quantity}"
                                       step="1">

      ...
      <div class="container">
         <nav>
            <ol class="breadcrumb">
               ...
               <li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/category?id=${requestScope.category.id}">${requestScope.category.name}</a>
               </li>
         ...
         <div class="row">
            ...
            <aside class="col-md-5 mb-md-0 mb-4 d-flex justify-content-center align-items-center">
               <c:choose>
                  <c:when test="${empty requestScope.product.imageName}">
                     <img width="280" height="280" class="img-fluid" src="${pageContext.request.contextPath}/img/280px.png" alt="280px.png">
                  </c:when>
                  <c:otherwise>
                     <img width="280" height="280" class="img-fluid" src="${pageContext.request.contextPath}/image/${requestScope.product.imageName}"
            ...
            <main class="col-md-7">
               ...
               <div class="rating-wrap my-3">
                  <span class="rating-stars me-2"> <c:forEach begin="1" end="5" var="i">
                        <i class="bi bi-star-fill ${i <= requestScope.averageRatingScore ? 'active' : ''}"></i>
                     </c:forEach>
                  </span> <small class="label-rating text-muted me-2">${requestScope.totalProductReviews} đánh giá</small> <small
                     class="label-rating text-success"><i class="bi bi-bag-check-fill"></i> ${requestScope.product.totalBuy} đã mua</small>
               </div>
               ...
               <div class="mb-4">
                  <c:choose>
                     <c:when test="${requestScope.product.discount == 0}">
                        <span class="price h4"> <fmt:formatNumber pattern="#,##0" value="${requestScope.product.price}" />₫
                        </span>
                     </c:when>
                     <c:otherwise>
                        ...
                        <span class="ms-2 badge bg-info">- <fmt:formatNumber pattern="#,##0" value="${requestScope.product.discount}" />%
                        </span>
                     </c:otherwise>
                  </c:choose>
               </div>
               ...
               <div class="d-flex align-items-center gap-2 mb-4">
                  ...
                  <form action="${pageContext.request.contextPath}/wishlist" method="post" class="d-inline">
                     ...
                     <c:choose>
                        <c:when test="${isInWishlist}">
                           ...
                           <button type="submit" class="btn btn-outline-danger" title="Xóa khỏi yêu thích">
                              <i class="bi bi-heart-fill"></i>
                           </button>
                        </c:when>
                        <c:otherwise>
                           <input type="hidden" name="action" value="add">
                           <button type="submit" class="btn btn-light" title="Thêm vào yêu thích">
                  ...
                  <form action="${pageContext.request.contextPath}/cartItem" method="post" class="d-flex align-items-center gap-2">
                     <input type="hidden" name="action" value="add"> <input type="hidden" name="userId" value="${sessionScope.currentUser.id}">
                     <input type="hidden" name="productId" value="${requestScope.product.id}">

                     <!-- Input số lượng -->
                     <input type="number" name="quantity" class="form-control w-auto" value="1" min="1" max="${requestScope.product.quantity}"
                        step="1">
            ...
            <div class="sin-rattings mb-4">
               <div class="star-author-all mb-2 clearfix">
                  <div class="ratting-author float-start">
                     ...
                     <span> <fmt:parseDate value="${productReview.createdAt}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedCreatedAt" type="both" />
         ...
         <c:choose>
            <c:when test="${not empty sessionScope.currentUser}">
               <form action="${pageContext.request.contextPath}/addProductReview" method="post">
                  ...
                  <input type="hidden" name="userId" value="${sessionScope.currentUser.id}"> <input type="hidden" name="productId"
            ...
            <c:otherwise>
               <p>
                  Vui lòng <a href="${pageContext.request.contextPath}/signin">đăng nhập</a> để đánh giá sản phẩm.
               </p>
            </c:otherwise>
         </c:choose>
      </div>
               ...
               <div class="col-xl-3 col-lg-4 col-md-6">
                  <div class="card p-3 mb-4">
                     <figure class="text-center">
                        <a href="${pageContext.request.contextPath}/product?id=${relatedProduct.id}" class="img-wrap"> <c:choose>
                        ...
                        <figcaption class="info-wrap mt-2">
                           <a href="${pageContext.request.contextPath}/product?id=${relatedProduct.id}" class="title">${relatedProduct.name}</a> ...
                        </figcaption>
                     </figure>
                  </div>
